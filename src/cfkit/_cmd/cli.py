...
from cfkit._utils.constants import ALL_ACTIONS, HELP_MESSAGE, LANGUAGES
...
def _check_problem_index(problem_index):
    ...
    try:
        if ((search(r"\A\d{1,4}[A-z]\d{,2}", problem_index)) is not None) or (
            (search(r"\A[A-z]{1}\d{,2}", problem_index)) is not None
        ):
            pass
...
def list_action():
    ...
    parser.add_argument(
        "-l", "--local", action="store", dest="local", help="Parse from html file"
    )
    ...
    print(
        f"{'Problem name':<{max_width}} {'Time Limit':<12} {'Memory Limit':<15} {'Input':<16} {'Output':<16}\n{'-' * 100}"
    )
    for problem in contest._content:
        for i in range(1, 5):
            problem[i] = problem[i][problem[i].find(": ") + 2 :]
        print(
            f"{problem[0]:<{max_width}} {problem[1]:<12} {problem[2]:<15} {problem[3]:<16} {problem[4]:<16}"
        )
...
def parse_action():
    ...
    parser.add_argument(
        ...
        dest="path",
        default=None,
        help="Path where samples will be created",
    )
    parser.add_argument(
        "-s",
        "--short",
        action="store_true",
        dest="short_names",
        help="Names samples as 'in' and 'out' (Works only when parsing a problem)",
    )
    parser.add_argument(
        "-l", "--local", action="store", dest="local", help="Parse from html file"
    )
    ...
    if contest_or_problem == "problem":
        Problem(problem_index, args.local).parse(
            args.path, args.create_tests_dir, args.short_names
        )
    else:
...
def gen_action():
    ...
    """
    from cfkit.codeforces._contest import Contest
    from cfkit.codeforces._problem import (
        EXTENSIONS,
    ...
    parser.add_argument(
        "code", action="store", help="Problem index (e.g. 4a) or file name (e.g. 4a.py)"
    )
    parser.add_argument(
        "-c",
        "--create",
        action="store_true",
        dest="create_contest_folder",
        help="Create contest folder",
    )
    parser.add_argument(
        "-e", "--ext", action="store", dest="ext", help="File extension"
    )
    parser.add_argument(
        "-n",
        "--name",
        action="store_true",
        dest="problem_name",
        help="Add problem name to file name",
    )
    parser.add_argument(
        "-p",
        "--path",
        ...
        dest="path",
        default=None,
        help="Path where file will be created",
    )
    parser.add_argument(
        "-l", "--local", action="store", dest="local", help="Parse from html file"
    )
    ...
    if (
        args.create_contest_folder is False
        and args.problem_name is False
        and args.code.isdigit() is False
    ):
        if args.code.find(".") == -1:
            if args.ext is None:
                ...
                if default_language == "":
                    ...
                    while args.ext not in EXTENSIONS:
                        colored_text(
                            "Extension is not recognised! Please try again",
                            one_color="error_4",
                        )
                        args.ext = input("Extension: ")
                else:
    ...
    if contest_problem == "problem":
        Problem(problem_index, args.local).create_solution_file(
            args.path, args.ext, args.create_contest_folder, args.problem_name
        )
    else:
        Contest(problem_index, args.local).create_problems_files(
            args.path, args.ext, args.problem_name, args.create_contest_folder
        )
...
def run_action():
    ...
    parser.add_argument(
        "-p", "--problem", action="store", dest="problem_code", help="Problem code"
    )
    parser.add_argument(
        "-o", "--order", action="store_true", dest="order", help="Any order"
    )
    parser.add_argument(
        "-c",
        "--custom",
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="remove",
        help="Remove samples and output files",
    )
    parser.add_argument(
        "-m",
        "--multiple-answers",
        action="store_true",
        dest="multiple_answers",
        help="There are multiple correct answers",
    )
    parser.add_argument(
        "-s",
        "--space",
        action="store_false",
        dest="ignore_extra_spaces",
        help="Does not ignore extra spaces",
    )
    parser.add_argument(
        "-n",
        "--newline",
        action="store_false",
        dest="ignore_extra_newlines",
        help="Does not ignore extra new lines",
    )
    parser.add_argument(
        "-v",
        "--verbose-off",
        action="store_false",
        dest="not_verbose",
        help="Does not print input, output and answer",
    )
    parser.add_argument(
        "-a",
        "--no-answers",
    ...
    if argv[1] == "edit":
        try:
            if operating_sys == "windows":
                run(
                    f'notepad "{Path.home().joinpath(".cfkit", "cfkit.conf")}"',
                    shell=True,
                )  # macOS
            elif operating_sys == "darwin":
                run(
                    f'open "{Path.home().joinpath(".cfkit", "cfkit.conf")}"', shell=True
                )  # macOS
            else:
                run(
                    f'xdg-open "{Path.home().joinpath(".cfkit", "cfkit.conf")}"',
                    shell=True,
                )  # Linux
        ...
        from cfkit._config.config import (
            set_language_attributes,
        )  # , set_default_submission_language
        ...
        def configure_mem_time_usage(mem_time_calc_exec):
            conf_file["cfkit"]["calculate_memory_usage_and_execution_time"] = (
                "yes"
                if confirm("Do you want to calculate memory usage and execution time?")
                else "no"
            )
            ...
            mem_time_calc_exec = source_dir.joinpath(
                "_dependencies", f"memory_time_usage_{mem_time_calc_exec}"
            )
        ...
        if arch in ("i386", "i686"):
            if operating_sys == "darwin":
                print(
                    "Unfortunately, memory and time tracking features are not supported on your current system configuration."
                )
            else:
...
def prepare_action():
    ...
    from cfkit._utils.common import insert_placeholders_template
    from cfkit._utils.file_operations import write_text_to_file
    from cfkit._utils.variables import template_folder
    ...
    args = parser.parse_args()
    template_name = f"{'atcoder' if args.type == 'atcoder' else 'default'}.cpp"
    template_path = template_folder / "cpp" / template_name
    template_text = template_path.read_text()
    contest_path = Path.cwd() / args.contest_id
    contest_path.mkdir(exist_ok=True, parents=True)

    PROBLEM_NUM = 7
    for i in range(PROBLEM_NUM):
        problem_path = contest_path / f"{chr(i + ord('A'))}.cpp"
        write_text_to_file(insert_placeholders_template(template_text), problem_path)
