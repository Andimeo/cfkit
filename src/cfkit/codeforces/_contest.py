...
from cfkit._utils.constants import EXTENSIONS, Directory
...
from cfkit._utils.parse_samples import fetch_samples, problems_content, samples_dir
from cfkit._utils.print import colored_text
from cfkit._utils.variables import conf_file, language_conf_path, resources_folder
...
class Contest:
    ...
    def __init__(self, contest_id: int, /, *args) -> None:
        ...
        self.problems_letters = list(
            map(lambda x: x[: x.find(".")], self.problems_names)
        )
    ...
    def __content(self, *args):
        ...
        if isinstance(self.contest_id, int):
            contest_problems_statement_file_path = resources_folder.joinpath(
                "problems", f"{self.contest_id}.txt"
            )
            if not contest_problems_statement_file_path.exists():
                ...
                response, self.name = problems_content(
                    contest_problems_statement_file_path, self.contest_id, *args
                )
            return response
        colored_text(
    ...
    def create_problems_files(
        ...
        programming_language_extension: Optional[str]
        | Optional[tuple]
        | Optional[list] = None,
        ...
        if (
            test := osPath.basename(path) != str(self.contest_id)
        ) and create_contest_folder:
        ...
        def enter_extensions(language_conf: dict) -> tuple[list, int]:
            ...
            programming_language_extension, problems_extensions_length = (
                check_for_undefined_extensions(input("Extension(s): ").strip().split())
            )
        ...
        def check_for_undefined_extensions(programming_language_extension: list):
            ...
            while not undefined_extension and i < problems_extensions_length:
                undefined_extension = (
                    EXTENSIONS.get(programming_language_extension[i]) is None
                )
                i += 1

            if undefined_extension:
                colored_text(
                    f"&apos;{programming_language_extension[i]}&apos; extension is not recognized"
                )
                return enter_extensions(language_conf)

            if problems_extensions_length > problems_num:
                print(
                    f"expected at most {problems_num} extensions, got {problems_extensions_length}."
                )
                return enter_extensions(language_conf)
            return programming_language_extension, problems_extensions_length
        ...
        if isinstance(programming_language_extension, (list, tuple)):
            programming_language_extension, problems_extensions_length = (
                check_for_undefined_extensions(programming_language_extension)
            )
            ...
            if EXTENSIONS.get(programming_language_extension) is None:
                ...
                programming_language_extension, problems_extensions_length = (
                    enter_extensions(read_json_file(language_conf_path))
                )
            else:
            ...
            if not default_language:
                programming_language_extension, problems_extensions_length = (
                    enter_extensions(language_conf)
                )
            else:
                programming_language_extension = [
                    language_conf[default_language]["extensions"][0]
                ]
                problems_extensions_length = 1
        ...
        if add_problem_name_to_file_name:
            ...
            for problem_name in self.problems_names:
                ...
                index = (i - problems_num + problems_extensions_length) + abs(
                ...
                problems_files.append(
                    file_name(
                        problem_name,
                        f"{contestid}{self.problems_letters[i]}",
                        programming_language_extension[index],
                    )
                )
                i += 1
            ...
            for problem_name in self.problems_names:
                ...
                index //= 2  # since (i - problems_num + problems_extensions_length) was added twice
                problems_files.append(
                    f"{contestid}{self.problems_letters[i]}.{programming_language_extension[index]}"
                )
                i += 1
        ...
        def create_solution_file(one_extension=True):
            def write_to_multiple_files_at_once(files, template):
                for file in files:
                    write_text_to_file(
                        insert_placeholders_template(read_text_from_file(template)),
                        file,
                    )

            if one_extension:
                write_to_multiple_files_at_once(
                    problems_files, retrieve_template(problems_files[0])
                )
            else:
                i = 0
                while i < problems_num:
                    ...
                    if i < aux:
                        write_to_multiple_files_at_once(
                            problems_files[i:aux], retrieve_template(problems_files[i])
                        )
                        i += aux
                    else:
                        template_source_code = read_text_from_file(
    ...
    def parse(
        self, path: Optional[Directory] = None, create_tests_dir: bool = True
    ) -> None:
        """
        Documentation
        """
        ...
        fetch_samples(
            ...
            path_to_save_samples=samples_dir(create_tests_dir, path),
